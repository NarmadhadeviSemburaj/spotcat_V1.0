...
('ZID_00002', 'North Zone', 'New York', '10001'),
('ZID_00003', 'North East Zone', 'Boston', '02108');

--
-- Triggers `zone`
--
DELIMITER $$
CREATE TRIGGER `after_zone_update` AFTER UPDATE ON `zone` FOR EACH ROW BEGIN
    IF NEW.zone_name != OLD.zone_name THEN
        UPDATE `line` 
        SET zone_name = NEW.zone_name 

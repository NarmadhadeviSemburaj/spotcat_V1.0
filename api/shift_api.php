...
function createShift($conn, $ip_address, $user_agent) {
    ...
    // Validate shift_name enum
    if (!in_array($input['shift_name'], ['Morning', 'Evening'])) {
        logUserAction(null, 'System', 'Create Shift', "Invalid shift_name value", 
                    $_SERVER['REQUEST_URI'], $_SERVER['REQUEST_METHOD'], 
                    $input, 'error', null, $ip_address, $user_agent);
        sendResponse('error', "shift_name must be either 'Morning' or 'Evening'", null, 400);
    }
...
function updateShift($conn, $ip_address, $user_agent) {
    ...
    try {
        ...
        if (isset($input['shift_name'])) {
            if (!in_array($input['shift_name'], ['Morning', 'Evening'])) {
                throw new Exception("shift_name must be either 'Morning' or 'Evening'");
            }
            $updates[] = "shift_name = ?";
